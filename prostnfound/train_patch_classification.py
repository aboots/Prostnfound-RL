...
def parse_args():
    ...
    group.add_argument("--use_augmentation", action="store_true", help="Use data augmentation")
...
def main(args):
    ...
    model = resnet10t(num_classes=2, in_chans=1)
    ...
    for epoch in range(args.epochs):
        ...
        wandb.log(
            dict(
                epoch=epoch,
                **{f"val_{k}": v for k, v in calculate_metrics(val_outputs).items()},
            )
        )
        accumulator.reset()
        model.eval()
...
def make_data_loaders(args):
    from src.dataset import BModePatchesDataset, Transform

    train_core_ids, val_core_ids, test_core_ids = select_cohort_from_args(args)
    ...
    train_dataset = BModePatchesDataset(
        ...
        transform=Transform(use_augmentations=args.use_augmentation),
    )
    print("Val dataset...")
