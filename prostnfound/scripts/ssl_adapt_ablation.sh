...
center=PCC
ssl_adapt=true
export WANDB_TAGS="test-$CENTER,ssl-ablation"
...
then 
  ...
  model_kw="${model_kw} enc_ckpt=/h/pwilson/projects/medAI/checkpoints/ibot/test_center_${center}.pt"
else
  model=prostnfound_adapter_medsam_legacy
fi 

CHECKPOINT=$CKPT_DIR srun -u python train.py \
  --splits_file test_center_${center}_no_downsample.json \
